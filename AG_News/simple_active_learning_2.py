...
def augment_with_llm(samples_df, tokenizer, model):
    ...
    for idx, (_, row) in enumerate(samples_df.iterrows(), 1):
        if verbose:
            print(f"  Augmenting sample {idx}/{total_samples}...", flush=True)
    ...
    try:
        ...
        if statistical_testing:
            ...
            active_results, active_final = run_active_learning_experiment(
                ...
                lambda_penalty=experiment_config.get("lambda_penalty", 0.0002),
            )
            passive_results, passive_final = run_passive_learning_experiment(
                X_train,
                y_train,
