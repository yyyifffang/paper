...
def uncertainty_sampling(model, X_unlabeled, n_samples):
    """主動學習：優先用 Entropy；若模型無機率輸出則用最小 margin。"""
    ...
    if hasattr(model, "predict_proba"):
        ...
        decision_scores = model.decision_function(X_unlabeled)
        if decision_scores.ndim == 1:
            # Binary case: 越接近 0 越不確定
            margins = np.abs(decision_scores)
        else:
            # Multi-class case: top-1 與 top-2 分數差越小越不確定
...
def train_and_evaluate(X_train, y_train, X_eval, y_eval):
    """訓練 LinearSVC 並回傳 weighted metrics。"""
...
def plot_utility_curve(passive_results, active_results, proposed_results, stopping_iters, config_name=""):
    ...
    utility_filename = _build_output_filename("utility_curve_comparison", config_name, "", "png")
    output_path = os.path.join(DATA_DIR, utility_filename)
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    show_plots = False
    config_name = "ledgar_warmup_llm_framework_svc"
