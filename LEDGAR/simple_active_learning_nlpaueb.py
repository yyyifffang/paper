...
CONTRACT_BERT_MODEL_ID = "nlpaueb/bert-base-uncased-contracts"
EXPERIMENT_NAME = "simple_active_learning_nlpaueb"


BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(BASE_DIR, "nlpaueb")
...
class ContractBertEmbedder:
    """nlpaueb 合約版 BERT 的 mean pooling 特徵提取器。"""

    def __init__(self, model_id=CONTRACT_BERT_MODEL_ID, max_length=512):
    ...
    def encode(
        ...
        result = np.vstack(embeddings).astype(np.float32) if embeddings else np.empty((0, 0), dtype=np.float32)
        if convert_to_numpy:
            return result
        return result
...
def load_ledgar_sentence_transformer(random_seed=42, return_texts=False, n_head=10, n_tail=10):
    """
    載入 LEDGAR 並轉成 nlpaueb 合約版 BERT embedding。
    ...
    text_encoder = ContractBertEmbedder(CONTRACT_BERT_MODEL_ID)
